...
class Cleanup(commands.Cog):
    ...
    async def pin(self, ctx, message_id):
        ...
        await modplus.notify('deletemessages', f"Message pin in {ctx.channel}")
    ...
    async def check_100_plus(ctx: commands.Context, number: int) -> bool:
        ...
        if not await ctx.bot.is_admin(ctx.author):
            await ctx.send("You cannot delete more than 50 messages. Please contact an administrator.")
            return False
        if ctx.assume_yes:
            return True
